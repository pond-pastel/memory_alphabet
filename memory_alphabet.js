...
let data, count, effectTimer, effectMoveTimerList;
...
function effectAdd() {
    ...
    effectMoveTimerList.push(setInterval(effectMove, 150, effect));
}

function effectMove(effect) {
    const windowHeight = window.innerHeight;
    const y = effect.getBoundingClientRect().top;
    if(y > windowHeight + (16 * 4)) {
        effect.remove(effect);
    } else {
        effect.style.top = `${y + 16}px`;
    }
}

function effectRemove(effect) {
    clearInterval(effectMoveTimerList.shift());
    effect.remove();
}
